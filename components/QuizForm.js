...
export default function QuizForm() {
  ...
  async function validateAndSubmit(e) {
    ...
    // Send the lead (with the test result) to Telegram via /api/lead.
    try {
      await fetch("/api/lead", {
        ...
        body: JSON.stringify({
          ...
          phone: contact.phone,
          source: "quiz",
          details: `Результат теста: ${result?.title ?? "—"} (баллы: ${score})`,
        }),
      });
    } catch {
      // Swallow — still confirm to the user; the call is best-effort.
    }
    setStep("done");
  }
